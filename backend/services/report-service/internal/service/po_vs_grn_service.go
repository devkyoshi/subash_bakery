...
func (s *POvsGRNService) GetPOvsGRNComparison(
	...
	// 7. Collect unique product IDs and resolve names/SKUs
	productIDs := make(map[string]bool)
	for _, po := range pos {
		for _, item := range po.Items {
			productIDs[item.ProductID.Hex()] = true
		}
	}
	productDetails := s.resolveProductDetails(ctx, productIDs)

	// 8. Build comparison items (one per PO item, aggregated across GRNs)
	items := s.buildComparisonItems(pos, grnMap, supplierNames, productDetails)
...
// productInfo holds resolved product name and SKU
type productInfo struct {
	Name string
	SKU  string
}

// resolveProductDetails fetches product names and SKUs from the products collection
func (s *POvsGRNService) resolveProductDetails(ctx context.Context, productIDs map[string]bool) map[string]productInfo {
	details := make(map[string]productInfo)

	for idStr := range productIDs {
		id, err := primitive.ObjectIDFromHex(idStr)
		if err != nil {
			continue
		}
		...
		err = s.productColl.FindOne(ctx, bson.M{"_id": id}).Decode(&product)
		if err != nil {
			details[idStr] = productInfo{Name: "Unknown Product", SKU: ""}
			continue
		}
		details[idStr] = productInfo{Name: product.Name, SKU: product.SKU}
	}
...
func (s *POvsGRNService) buildComparisonItems(
	...
	for _, po := range pos {
		...
		for _, poItem := range po.Items {
			...
			// Determine status
			status := "PENDING"
			if totalReceived == 0 && po.Status != sharedModels.POStatusReceived {
				status = "PENDING"
			...
			// Resolve product name and SKU from products collection
			prodID := poItem.ProductID.Hex()
			prodInfo, ok := productDetails[prodID]

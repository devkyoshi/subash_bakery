...
// ============================================================
// Stock Level Report — Excel
// ============================================================

// GenerateStockLevelExcel creates an Excel file from stock level data
func (s *ExportService) GenerateStockLevelExcel(
	items []models.StockLevelComparisonItem,
	...
	f.SetCellValue(summarySheet, "A1", "Stock Level Comparison Report")
	f.SetCellValue(summarySheet, "A2", fmt.Sprintf("Generated: %s", time.Now().Format("02 Jan 2006, 03:04 PM")))
	f.SetCellValue(summarySheet, "A3", fmt.Sprintf("Total Products: %d", metrics.TotalProducts))

	// Metrics
	metricHeaders := []string{"Metric", "Value"}
	for i, h := range metricHeaders {
		cell := fmt.Sprintf("%c5", 'A'+i)
		f.SetCellValue(summarySheet, cell, h)
	}

	metricsRows := [][]interface{}{
		{"Total Products", metrics.TotalProducts},
		{"Optimal", metrics.OptimalCount},
		{"Low Stock", metrics.LowStockCount},
		{"Critical", metrics.CriticalCount},
		{"Overstock", metrics.OverstockCount},
		{"Out of Stock", metrics.OutOfStockCount},
		{"Total Stock Value", metrics.TotalStockValue},
		{"Total On Hand", metrics.TotalOnHand},
		{"Total Allocated", metrics.TotalAllocated},
		{"Total Available", metrics.TotalAvailable},
	}
	...
	headerStyle, _ := f.NewStyle(&excelize.Style{
		...
		Fill:      excelize.Fill{Type: "pattern", Pattern: 1, Color: []string{"#374150"}},
		Alignment: &excelize.Alignment{Horizontal: "center", Vertical: "center"},
	})
	f.SetCellStyle(summarySheet, "A5", "B5", headerStyle)

	// ====== Detail Sheet ======
	detailSheet := "Stock Levels"
	f.NewSheet(detailSheet)

	headers := []string{"SKU", "Product Name", "Category", "Location", "Unit",
		"On Hand", "Available", "Allocated", "In Transit",
		"Reorder Level", "Min Stock", "Max Stock",
		"Avg Cost", "Total Value", "Status"}

	for i, h := range headers {
		cell := fmt.Sprintf("%c1", 'A'+i)
		f.SetCellValue(detailSheet, cell, h)
	}
	f.SetCellStyle(detailSheet, "A1", fmt.Sprintf("%c1", 'A'+len(headers)-1), headerStyle)

	for i, item := range items {
		row := i + 2
		f.SetCellValue(detailSheet, fmt.Sprintf("A%d", row), item.SKU)
		f.SetCellValue(detailSheet, fmt.Sprintf("B%d", row), item.ProductName)
		f.SetCellValue(detailSheet, fmt.Sprintf("C%d", row), item.CategoryName)
		f.SetCellValue(detailSheet, fmt.Sprintf("D%d", row), item.LocationName)
		f.SetCellValue(detailSheet, fmt.Sprintf("E%d", row), item.Unit)
		f.SetCellValue(detailSheet, fmt.Sprintf("F%d", row), item.SystemQty)
		f.SetCellValue(detailSheet, fmt.Sprintf("G%d", row), item.AvailableQty)
		f.SetCellValue(detailSheet, fmt.Sprintf("H%d", row), item.AllocatedQty)
		f.SetCellValue(detailSheet, fmt.Sprintf("I%d", row), item.InTransitQty)
		f.SetCellValue(detailSheet, fmt.Sprintf("J%d", row), item.ReorderLevel)
		f.SetCellValue(detailSheet, fmt.Sprintf("K%d", row), item.MinStock)
		f.SetCellValue(detailSheet, fmt.Sprintf("L%d", row), item.MaxStock)
		f.SetCellValue(detailSheet, fmt.Sprintf("M%d", row), item.AverageCost)
		f.SetCellValue(detailSheet, fmt.Sprintf("N%d", row), item.TotalValue)
		f.SetCellValue(detailSheet, fmt.Sprintf("O%d", row), item.StockStatus)
	}

	// Auto-width
	for i := range headers {
		col := string(rune('A' + i))
		f.SetColWidth(detailSheet, col, col, 14)
	}
	f.SetColWidth(detailSheet, "B", "B", 25)
	f.SetColWidth(detailSheet, "C", "C", 18)
	f.SetColWidth(detailSheet, "D", "D", 18)
...
// GenerateStockLevelPDF creates a branded PDF for stock level data
func (s *ExportService) GenerateStockLevelPDF(
	items []models.StockLevelComparisonItem,
	metrics *models.StockLevelMetrics,
	filters models.StockLevelFilters,
) (*bytes.Buffer, error) {
	...
	// Brand colors (same as PO vs GRN)
	brandR, brandG, brandB := 180, 60, 40
	accentR, accentG, accentB := 240, 130, 60
	darkR, darkG, darkB := 45, 45, 50
	mutedR, mutedG, mutedB := 120, 120, 130
	headerR, headerG, headerB := 55, 65, 80
	bgLightR, bgLightG, bgLightB := 250, 248, 245

	// ============================================================
	// Reusable header/footer (branded template)
	// ============================================================
	drawPageHeader := func() {
		pdf.SetFillColor(brandR, brandG, brandB)
		pdf.Rect(0, 0, pageW, 4, "F")
		...
		pdf.CellFormat(78, 4, fmt.Sprintf("Generated: %s", time.Now().Format("02 Jan 2006, 03:04 PM")), "", 0, "R", false, 0, "")
		pdf.SetXY(pageW-90, 14)
		pdf.CellFormat(78, 4, fmt.Sprintf("Total Products: %d", len(items)), "", 0, "R", false, 0, "")
	...
	pdf.SetFont("Arial", "B", 22)
	pdf.SetTextColor(darkR, darkG, darkB)
	pdf.CellFormat(0, 12, "Stock Level Comparison Report", "", 1, "L", false, 0, "")
	pdf.Ln(1)

	pdf.SetFillColor(bgLightR, bgLightG, bgLightB)
	pdf.SetFont("Arial", "", 9)
	pdf.SetTextColor(mutedR, mutedG, mutedB)
	pdf.CellFormat(0, 7, "    Current inventory stock level analysis across all locations", "", 1, "L", true, 0, "")
	pdf.Ln(6)
	...
	kpis := []kpiCard{
		...
		{"Optimal", fmt.Sprintf("%d", metrics.OptimalCount), "Healthy stock", 34, 160, 90},
		{"Low Stock", fmt.Sprintf("%d", metrics.LowStockCount), "Below reorder", 220, 160, 40},
		{"Critical", fmt.Sprintf("%d", metrics.CriticalCount), "Below minimum", 220, 70, 55},
		{"Overstock", fmt.Sprintf("%d", metrics.OverstockCount), "Above maximum", 59, 130, 246},
		{"Out of Stock", fmt.Sprintf("%d", metrics.OutOfStockCount), "Zero quantity", 107, 114, 128},
	}
	...
	pdf.SetFont("Arial", "B", 11)
	pdf.SetTextColor(darkR, darkG, darkB)
	pdf.CellFormat(0, 8, "Inventory Valuation Summary", "", 1, "L", false, 0, "")
	pdf.Ln(1)

	finColW := float64(135)
	finRows := [][]string{
		{"Total Stock Value", fmt.Sprintf("%.2f", metrics.TotalStockValue)},
		{"Total Quantity On Hand", fmt.Sprintf("%.2f", metrics.TotalOnHand)},
		{"Total Available", fmt.Sprintf("%.2f", metrics.TotalAvailable)},
		{"Total Allocated", fmt.Sprintf("%.2f", metrics.TotalAllocated)},
	}
	...
	pdf.SetFont("Arial", "B", 14)
	pdf.SetTextColor(darkR, darkG, darkB)
	pdf.CellFormat(0, 10, "Detailed Stock Levels", "", 1, "L", false, 0, "")
	pdf.Ln(2)

	// Table columns — must fit within 273mm (297 - 12*2)
	colWidths := []float64{20, 42, 30, 30, 14, 22, 22, 22, 20, 20, 22, 29}
	colHeaders := []string{"SKU", "Product", "Category", "Location", "Unit",
		"On Hand", "Available", "Allocated", "Reorder", "Avg Cost", "Value", "Status"}
	...
	for i, item := range items {
		if pdf.GetY() > pageH-25 {
			...
			pdf.SetFont("Arial", "B", 11)
			pdf.SetTextColor(darkR, darkG, darkB)
			pdf.CellFormat(0, 8, "Detailed Stock Levels (continued)", "", 1, "L", false, 0, "")
			pdf.Ln(1)
			drawTableHeader()
		}
		...
		pdf.SetTextColor(darkR, darkG, darkB)
		pdf.CellFormat(colWidths[0], 5.5, item.SKU, "1", 0, "L", fill, 0, "")

		productName := item.ProductName
		if len(productName) > 24 {
			productName = productName[:22] + ".."
		}
		pdf.CellFormat(colWidths[1], 5.5, productName, "1", 0, "L", fill, 0, "")

		catName := item.CategoryName
		if len(catName) > 16 {
			catName = catName[:14] + ".."
		}
		pdf.CellFormat(colWidths[2], 5.5, catName, "1", 0, "L", fill, 0, "")

		locName := item.LocationName
		if len(locName) > 16 {
			locName = locName[:14] + ".."
		}
		pdf.CellFormat(colWidths[3], 5.5, locName, "1", 0, "L", fill, 0, "")

		pdf.CellFormat(colWidths[4], 5.5, item.Unit, "1", 0, "C", fill, 0, "")
		pdf.CellFormat(colWidths[5], 5.5, fmt.Sprintf("%.2f", item.SystemQty), "1", 0, "R", fill, 0, "")
		pdf.CellFormat(colWidths[6], 5.5, fmt.Sprintf("%.2f", item.AvailableQty), "1", 0, "R", fill, 0, "")
		pdf.CellFormat(colWidths[7], 5.5, fmt.Sprintf("%.2f", item.AllocatedQty), "1", 0, "R", fill, 0, "")
		pdf.CellFormat(colWidths[8], 5.5, fmt.Sprintf("%d", item.ReorderLevel), "1", 0, "R", fill, 0, "")
		pdf.CellFormat(colWidths[9], 5.5, fmt.Sprintf("%.2f", item.AverageCost), "1", 0, "R", fill, 0, "")
		pdf.CellFormat(colWidths[10], 5.5, fmt.Sprintf("%.2f", item.TotalValue), "1", 0, "R", fill, 0, "")
		...
		pdf.SetFont("Arial", "B", 6.5)
		pdf.CellFormat(colWidths[11], 5.5, item.StockStatus, "1", 0, "C", true, 0, "")
		pdf.SetFont("Arial", "", 7)
		pdf.Ln(-1)
	}
	...
	pdf.SetFillColor(headerR, headerG, headerB)
	pdf.SetTextColor(255, 255, 255)
	firstCols := colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4]
	pdf.CellFormat(firstCols, 6, fmt.Sprintf("  TOTAL (%d items)", len(items)), "1", 0, "L", true, 0, "")

	var totalOnHand, totalAvail, totalAlloc float64
	var totalVal float64
	for _, item := range items {
		totalOnHand += item.SystemQty
		totalAvail += item.AvailableQty
		totalAlloc += item.AllocatedQty
		totalVal += item.TotalValue
	}
	pdf.CellFormat(colWidths[5], 6, fmt.Sprintf("%.2f", totalOnHand), "1", 0, "R", true, 0, "")
	pdf.CellFormat(colWidths[6], 6, fmt.Sprintf("%.2f", totalAvail), "1", 0, "R", true, 0, "")
	pdf.CellFormat(colWidths[7], 6, fmt.Sprintf("%.2f", totalAlloc), "1", 0, "R", true, 0, "")
	pdf.CellFormat(colWidths[8], 6, "", "1", 0, "C", true, 0, "")
	pdf.CellFormat(colWidths[9], 6, "", "1", 0, "C", true, 0, "")
	pdf.CellFormat(colWidths[10], 6, fmt.Sprintf("%.2f", totalVal), "1", 0, "R", true, 0, "")
	pdf.CellFormat(colWidths[11], 6, "", "1", 0, "C", true, 0, "")
	pdf.Ln(-1)
